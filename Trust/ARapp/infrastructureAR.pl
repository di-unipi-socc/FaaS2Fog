...
%privateCitzen
node(private1, privateCitzen, [], [java], (1024,4,2500), [privatePrices]).
node(private2, privateCitzen, [data_encryption], [python], (512,2,1500), [privatePrices]).
%telco
...
%eventGenerator(generatorId, sourceType, event)
eventGenerator(userDevice, newScreen).
eventGenerator(userDevice, newMapReq).
